...
UTF8::String UTF8::String::Insert(unsigned int Start, const String &Insert) const
{
	return SubstringReplace(Start, 0, Insert);
}
...
UTF8::String::String(int size, char ch)
{
    InitString();
    ...
    if (s) {
        ...
        for (int i = 0; i < WideStringLength; i++) {
            ...
            if (wc < 0x80) {
                ...
                tmp[pos++] = (wc & /*0b111111*/0x3F) | 0x80/*0b10000000*/;
            } else if (wc < 0x10000) {
                tmp[pos++] = (wc >> 12) | 0xE0/*0b11100000*/;
                tmp[pos++] = ((wc >> 6) & 0x3F/*0b111111*/) | 0x80/*0b10000000*/;
                tmp[pos++] = (wc & 0x3F/*0b111111*/) | 0x80/*0b10000000*/;
            } else {
                tmp[pos++] = (wc >> 18) | 0xF0/*0b11110000*/;
                tmp[pos++] = ((wc >> 12) & 0x3F/*0b111111*/) | 0x80/*0b10000000*/;
                tmp[pos++] = ((wc >> 6) & 0x3F/*0b111111*/) | 0x80/*0b10000000*/;
                tmp[pos++] = (wc & 0x3F/*0b111111*/) | 0x80/*0b10000000*/;
            }
    ...
    if (str) {
        ...
        while (str[n]) {
            ...
            for (i = 1; i < SequenceLength; i++) {
                if ((((unsigned char) str[n + i]) >> 6) != 2/*0b10*/) {
                    std::string s(str);
...
unsigned int UTF8::String::GetSequenceLength(const char * StartByte) const {
    if (StartByte && strlen(StartByte)) {
        ...
        if ((Byte >> 4) == 0x0E/*0b1110*/) {
            return 3;
        }

        if ((Byte >> 3) == 0x1E/*0b11110*/) {
